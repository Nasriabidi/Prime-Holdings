...
<b>Account Information</b> section.
                ...
                <template v-else-if="step === 2">
                  <div class="text-center">
                    ...
                    <select v-model="selectedCrypto" style="color: black;" class="w-full px-3 py-2 border rounded focus:outline-none focus:ring focus:border-blue-300 mb-4 cursor-pointer">
                      <option class="cursor-pointer" style="color: black;" value="">-- Choose --</option>
                      <option class="cursor-pointer" style="color: black;" v-for="crypto in cryptoList" :key="crypto.symbol" :value="crypto.symbol">
                        {{ crypto.name }} ({{ crypto.symbol }})
                      </option>
                    </select>
                    <button @click="showWalletStep" :disabled="!selectedCrypto" class="w-full bg-green-600 text-white py-2 rounded hover:bg-green-700 transition mb-4">Next</button>
                    <p v-if="error" class="text-red-500 mt-2">{{ error }}</p>
                  </div>
                </template>
                <template v-else-if="step === 3">
                  <div class="text-center">
                    <p class="mb-2">{{ selectedCryptoName }} Wallet Address:</p>
                    ...
                    <transition name="fade">
                      <div v-if="showNetworkWarning" class="mb-3 px-4 py-3 rounded bg-green-100 text-green-800 font-semibold text-center border border-green-300">
                        Please ensure that you only send {{ selectedCryptoName }} via the {{ selectedCryptoNetwork }} network. Sending via an unsupported network may result in the permanent loss of your funds.
                      </div>
                    </transition>
                    ...
                    <div class="trading-wrap overflow-x-auto">
                      <div class="responsive-wrap min-w-[900px]">
                        ...
                        <div v-for="(item, idx) in rechargeHistory.filter(h => h.visible)" :key="idx" class="content grid items-center grid-cols-[50px_1fr_1.5fr_1fr_1fr_1fr] gap-[10px] border-b border-[#000]/10 py-[10px]">
                          ...
                          <p class="text-[14px] text-dark leading-[1.5] tracking-[-0.05px] dark:text-white">{{ item.amount }} $</p>
...
const selectedCryptoNetwork = computed(() => {
  const found = cryptoList.find(c => c.symbol === selectedCrypto.value);
  return found ? found.network : '';
});
const loading = ref(false);
const error = ref('');
const refundWalletAddress = ref('');
...
function showWalletStep() {
  ...
  const found = cryptoList.find(c => c.symbol === selectedCrypto.value);
  if (!found) {
    error.value = 'Please select a cryptocurrency.';
    return;
  }
  refundWalletAddress.value = found.address;
  step.value = 3;
}
...
/* Make cursor pointer on select options for crypto selection */
select option {
  cursor: pointer;
}
</style>
