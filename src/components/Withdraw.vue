...
const withdrawMethod = ref(''); // 'bank' or 'crypto'
const cryptoWalletAddress = ref('');
...
async function handleWithdrawConfirm() {
  ...
  } else if (withdrawMethod.value === 'crypto') {
    if (!cryptoWalletAddress.value) {
      withdrawError.value = 'Please enter your crypto wallet address.';
      return;
    }
    try {
      await addDoc(collection(db, 'WithdrawRequest'), {
    ...
    <div>
      <div v-if="showWithdrawModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
        <div class="bg-white dark:bg-toggle rounded-2xl shadow-2xl p-8 w-full max-w-md relative border border-gray-100 dark:border-gray-700">
          ...
          <div v-if="withdrawStep === 0">
            <p class="mb-6 text-center text-lg font-semibold text-dark dark:text-white">Choose withdrawal method:</p>
            <div class="flex flex-col gap-4">
              <button @click="withdrawMethod = 'bank'; withdrawStep = 1;" class="w-full bg-primary text-white py-3 rounded-lg font-bold text-lg shadow-md hover:bg-primary/90 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">Bank Transfer</button>
          ...
          <form v-else-if="withdrawStep === 1 && withdrawMethod === 'crypto'" @submit.prevent="handleWithdrawConfirm" class="space-y-5">
            ...
            <div>
              <label class="block text-sm font-semibold text-dark dark:text-white mb-1">Wallet Address</label>
              <input type="text" v-model="cryptoWalletAddress"
                class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 px-4 py-3 text-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition placeholder-gray-400 dark:placeholder-gray-500"
                placeholder="Enter your external crypto wallet address" />
            </div>
            <div v-if="withdrawError" class="text-red-500 text-sm text-center">{{ withdrawError }}</div>
            <button type="submit"
              class="w-full bg-primary text-white py-3 rounded-lg font-bold text-lg shadow-md hover:bg-primary/90 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
