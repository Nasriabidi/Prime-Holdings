  ...
  <div class="admin-dashboard min-h-screen flex flex-col md:flex-row">
    ...
    <main class="flex-1 p-4 md:p-8">
      ...
      <section v-if="activeSection === 'withdrawControl'" id="withdraw-control" class="mb-4">
        ...
        <div v-else>
          <div v-if="withdrawHistory.filter(item => item.method === 'bank').length">
            <h3 class="font-bold text-lg mb-2">Bank Transfer Requests</h3>
            <div class="overflow-x-auto mb-6">
              <table class="min-w-full border text-sm">
                ...
                <tbody>
                  <tr v-for="(item, idx) in withdrawHistory.filter(item => item.method === 'bank')" :key="item.id">
                    <td class="border px-2 py-1">{{ idx + 1 }}</td>
          ...
          <div v-if="withdrawHistory.filter(item => item.method === 'crypto').length">
            <h3 class="font-bold text-lg mb-2">Crypto Wallet Requests</h3>
            <div class="overflow-x-auto">
              <table class="min-w-full border text-sm">
                ...
                <tbody>
                  <tr v-for="(item, idx) in withdrawHistory.filter(item => item.method === 'crypto')" :key="item.id">
                    <td class="border px-2 py-1">{{ idx + 1 }}</td>
                    <td class="border px-2 py-1">${{ Number(item.amount).toFixed(2) }}</td>
                    <td class="border px-2 py-1">
                      <span :class="{
        ...
        <div v-if="selectedSessionUser && filteredLiveSessions.length > 0" class="mb-6 p-4 bg-gray-50 dark:bg-dark rounded border border-gray-200 dark:border-gray-700 max-w-xl">
          ...
          <div class="flex flex-col sm:flex-row gap-4 items-end">
            ...
            <div>
              ...
              <input type="number" v-model.number="calcProfit" class="border rounded px-2 py-1 w-32" style="color: black;" />
            </div>
            <div>
          ...
          <table class=" border text-sm">
            ...
            <tbody>
              <tr v-for="session in filteredLiveSessions" :key="session.id">
                <td class="border px-2 py-1">
                  <input type="number" v-model.number="session._editProfit" style="color: black;" class="border rounded px-1 py-0.5 w-20" />
                </td>
                <td class="border px-2 py-1">
                  <input type="number" v-model.number="session._editFinalPrice" style="color: black;" class="border rounded px-1 py-0.5 w-20" />
                </td>
                <td class="border px-2 py-1">
      ...
      <section v-if="activeSection === 'notification'" id="notification-sender" class="mb-4">
        ...
        <div class="mb-4">
          ...
          <textarea v-model="message" class="border rounded px-2 py-1 w-full max-w-lg h-24" style="color: black;" placeholder="Enter notification message..."></textarea>
        </div>
